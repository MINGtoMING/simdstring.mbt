...
export int64_t find_char(const struct moonbit_view_t haystack, const uint32_t needle)
{
...
export int64_t find_char(const struct moonbit_view_t haystack, const uint32_t needle)
{
    ...
    if (likely(needle <= 0xFFFF))
    {
        ...
        while (likely(ptr <= end - 16))
        {
            ...
            if (unlikely(res_mask))
                return (int32_t)(ptr - start) + ctz32(res_mask) / 2;
            ptr += 16;
        }
    ...
    if (likely(needle <= 0xFFFF))
    {
        ...
        while (likely(ptr <= end - 8))
        {
            ...
            if (unlikely(vmaxvq_u16(res_blk)))
                return (int32_t)(ptr - start) + ctz64(to_bitmask(res_blk)) / 8;
            ptr += 8;
        }
...
export int64_t find_char(const struct moonbit_view_t haystack, const uint32_t needle)
{
    ...
    if (likely(needle <= 0xFFFF))
    {
        ...
        while (likely(ptr <= end - 4))
        {
            ...
            if (unlikely(res_mask))
                return (int32_t)(ptr - start) + ctz64(res_mask) / 16;
            ptr += 4;
        }
